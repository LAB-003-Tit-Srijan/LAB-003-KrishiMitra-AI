...
type Lecture = { _id: string; title: string; sourceUrl?: string };
 ...
 useEffect(() => {
  ...
  return (
    <div className="flex h-full min-h-[320px] flex-col gap-3">
      <h3 className="text-lg font-medium text-zinc-100">AI tutor</h3>
      {!tutorOk && (
        <p className="rounded-lg border border-white/10 bg-white/5 px-2 py-2 text-xs text-zinc-400">
          AI tutor is limited for this material. Playback and transcript remain available; use AI summary for a
          concise overview.
        </p>
      )}
      <p className="text-xs text-zinc-500">Tip: try “Explain 12:40” or “what happens at 1:05:00?”</p>
      <textarea
        value={q}
      ...
      {refs.length > 0 && (
        <div className="text-xs text-zinc-400">
          <p className="mb-1 font-medium text-zinc-300">References</p>
...
export default function PlayerPage() {
  ...
  const [transcript, setTranscript] = useState<
    Array<{ id: string; text: string; startSec: number; endSec: number }>
  >([]);
  const [timeline, setTimeline] = useState<
    Array<{ id: string; timestamp: number; label: string; importance: number }>
  >([]);
  ...
  useEffect(() => {
    ...
    Promise.all([api.get(`/lectures/${lectureId}`), api.get(`/timeline?lectureId=${lectureId}`)]).then(
      ([lectureRes, timelineRes]) => {
        const lecture = lectureRes.data?.lecture;
        setInsight((lectureRes.data?.educationalInsight as EducationalInsight) ?? null);
        const chunks = lectureRes.data?.transcript?.chunks || [];
        setVideoUrl(
          lecture?.sourceUrl ||
            "https://interactive-examples.mdn.mozilla.net/media/cc0-videos/flower.mp4"
        );
        setTranscript(
          chunks.map((c: { embeddingId?: string; text: string; startSec: number; endSec: number }, idx: number) => ({
  ...
  /** Passive watch-time heartbeat while a lecture is selected (Learn page). */
  useEffect(() => {
    if (!lectureId) return;
    const tick = () => {
      api.post("/study/watch-time", { seconds: 45 }).catch(() => {});
    };
  ...
  async function run(action: "summary" | "quiz" | "reco") {
    if (!lectureId) return;
    setBusy(action);
    try {
      if (action === "summary") {
        const { data } = await api.post("/learning/summary", { lectureId });
        setSummary(data.summary || "");
      } else if (action === "quiz") {
        const { data } = await api.post("/quiz/generate", { lectureId });
        setQuizJson(JSON.stringify(data.questions || [], null, 2));
      } else {
        const { data } = await api.post("/learning/recommendations", { lectureId });
        setReco(data.recommendations || "");
      }
    } catch {
      /* axios surfaces 403 in UI elsewhere */
    } finally {
      setBusy(null);
    }
  }
  ...
  return (
    <div>
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div className="flex-1 space-y-3">
            ...
            <div className="flex flex-wrap items-center gap-3">
              <label className="text-sm text-zinc-400">Lecture</label>
          ...
          <div className="flex flex-wrap gap-2">
            ...
            >
              {busy === "summary" ? "…" : "AI summary"}
            </button>
            <button
              type="button"
              disabled={!lectureId || busy !== null || !quizOk}
              title={!quizOk ? "Quiz generation needs strongly educational content." : undefined}
              onClick={() => run("quiz")}
              className="rounded-xl border border-white/15 bg-white/5 px-3 py-2 text-sm hover:bg-white/10 disabled:opacity-40"
            >
              {busy === "quiz" ? "…" : "Generate quiz"}
            </button>
            <button
              type="button"
              disabled={!lectureId || busy !== null || !recoOk}
              title={!recoOk ? "Adaptive study plan needs strongly educational content." : undefined}
              onClick={() => run("reco")}
              className="rounded-xl border border-white/15 bg-white/5 px-3 py-2 text-sm hover:bg-white/10 disabled:opacity-40"
            >
              {busy === "reco" ? "…" : "Study plan"}
            </button>
          </div>
        </div>
          ...
          <div className="glass grid gap-4 rounded-2xl p-4 lg:grid-cols-3">
            {summary && (
              <div className="lg:col-span-1">
                <p className="text-xs font-medium uppercase tracking-wide text-cyan-400">Summary</p>
                <pre className="mt-2 max-h-56 overflow-auto whitespace-pre-wrap text-xs text-zinc-300">{summary}</pre>
              </div>
            )}
            {quizJson && (
              <div className="lg:col-span-1">
                <p className="text-xs font-medium uppercase tracking-wide text-purple-400">Quiz (JSON)</p>
                <pre className="mt-2 max-h-56 overflow-auto text-xs text-zinc-300">{quizJson}</pre>
              </div>
            )}
            {reco && (
              <div className="lg:col-span-1">
                <p className="text-xs font-medium uppercase tracking-wide text-amber-400">Recommendations</p>
                <pre className="mt-2 max-h-56 overflow-auto whitespace-pre-wrap text-xs text-zinc-300">{reco}</pre>
              </div>
            )}
          </div>
        )}
      ...
      <SmartPlayer
        ...
        timeline={timeline}
        notebookLayout
        chatSlot={<TutorSidebar lectureId={lectureId} insight={insight} />}
      />
    </div>
  );
}
